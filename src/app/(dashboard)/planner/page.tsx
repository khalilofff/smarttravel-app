      ...
      {result && (
        <div className="space-y-5">
          ...
          <Card className="border-primary/40 bg-primary/5"><CardHeader><CardTitle className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5" /> AI selected optimal plan</CardTitle></CardHeader><CardContent className="space-y-4">
            ...
            {displayBudget?.aiSummary ? <div className={`hidden sm:block rounded-xl border p-3 text-sm ${displayBudget.affordabilityGap ? "border-yellow-500/50 bg-yellow-500/5" : "bg-muted/20"}`}><b>AI budget decision:</b> {String(displayBudget.aiSummary)}</div> : null}
            {result.alternativePlans?.length ? <div className="mobile-plan-chips grid grid-cols-3 md:grid-cols-3 gap-2">{result.alternativePlans.map((plan: any) => <button type="button" key={plan.type} className="rounded-xl border p-2 text-left text-xs bg-card hover:bg-muted/40"><p className="font-semibold truncate">{plan.title}</p><p className="mt-1 font-bold">{formatCurrency(Number(plan.budget?.flights || 0) + Number(plan.budget?.hotels || 0) + Number(plan.budget?.food || 0) + Number(plan.budget?.transport || 0) + Number(plan.budget?.activities || 0) + Number(plan.budget?.shopping || 0) + Number(plan.budget?.emergency || 0), form.currency)}</p><p className="hidden sm:block text-xs text-muted-foreground mt-1">{plan.aiReason}</p></button>)}</div> : null}
            {result.recommendations?.length ? <details className="hidden sm:block rounded-xl border bg-muted/10 p-3"><summary className="cursor-pointer text-sm font-semibold">AI notes</summary><div className="grid md:grid-cols-2 gap-2 mt-3">{result.recommendations.map((x: string) => <div key={x} className="rounded-xl border p-3 text-sm">{x}</div>)}</div></details> : null}
          </CardContent></Card>

          <Card><CardHeader><CardTitle className="flex items-center gap-2"><Plane className="h-5 w-5" /> Flight options</CardTitle></CardHeader><CardContent className="space-y-3"><div className="rounded-2xl border bg-muted/20 p-3 sm:p-4 space-y-3"><div className="grid md:grid-cols-2 xl:grid-cols-6 gap-3"><div><Label>Sort</Label><Select value={draftFlightSort} onChange={(e: any) => setDraftFlightSort(e.target.value)} className="mt-1"><option value="AI_BEST">AI best</option><option value="CHEAPEST">Cheapest</option><option value="FASTEST">Fastest</option><option value="FEWEST_STOPS">Fewest stops</option></Select></div><div><Label>Stops</Label><Select value={draftMaxStops} onChange={(e: any) => setDraftMaxStops(e.target.value)} className="mt-1"><option value="ANY">Any</option><option value="0">Direct only</option><option value="1">Max 1 stop</option><option value="2">Max 2 stops</option></Select></div><div><Label>Airline</Label><Select value={draftAirlineFilter} onChange={(e: any) => setDraftAirlineFilter(e.target.value)} className="mt-1"><option value="ALL">All airlines</option>{availableAirlines.map((name) => <option key={name} value={name}>{name}</option>)}</Select></div><div><Label>Departure</Label><Select value={draftDepartureTimeFilter} onChange={(e: any) => setDraftDepartureTimeFilter(e.target.value)} className="mt-1"><option value="ANY">Any time</option><option value="MORNING">Morning</option><option value="AFTERNOON">Afternoon</option><option value="EVENING">Evening</option><option value="NIGHT">Night</option></Select></div><div><Label>Max budget</Label><Input type="number" value={draftMaxFlightBudget} onChange={(e) => setDraftMaxFlightBudget(e.target.value)} placeholder="No limit" className="mt-1" /></div><div className="flex items-end"><label className="flex items-center gap-2 text-sm rounded-xl border px-3 py-2 h-10 w-full"><input type="checkbox" checked={draftDirectOnly} onChange={(e) => setDraftDirectOnly(e.target.checked)} /> Direct only</label></div></div><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mobile-wrap"><p className="text-xs text-muted-foreground">Showing only the {filteredFlights.length} flight option(s) matching the applied filters out of {result.flights?.length || 0}. Change fields, then press Apply.</p><div className="grid grid-cols-1 gap-2 sm:flex"><Button type="button" size="sm" className="w-full sm:w-auto" onClick={applyFlightFilters}>Apply filters</Button><Button type="button" size="sm" variant="outline" className="w-full sm:w-auto" onClick={resetFlightFilters}>Reset filters</Button></div></div></div>{filteredFlights?.length ? filteredFlights.map((f: any, i: number) => { const active = selectedFlightId === f.id; const recommended = result.recommendedFlightId === f.id; return <div key={f.id || i} className={`mobile-result-card rounded-2xl border p-4 sm:p-5 grid lg:grid-cols-[1fr_auto] gap-3 sm:gap-4 transition-colors mobile-wrap ${active ? "border-primary bg-primary/10" : "hover:bg-muted/30"}`}>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><p className="mobile-result-title font-semibold text-base sm:text-lg break-words">{f.route}</p><Badge variant="secondary">{f.source || result.flightSource || "Live API"}</Badge><Badge variant="outline">{f.cabinClass || form.flightCabin?.replace("_", " ") || "Economy"}</Badge>{recommended && <Badge variant="success">AI recommended</Badge>}{active && <Badge variant="default">Selected</Badge>}</div>
              {f.outbound ? (
                <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                  <div className="mobile-segment-card rounded-xl border bg-card/60 p-2.5 sm:p-3">
                    <p className="font-semibold text-foreground mb-2">Outbound</p>
                    <div className="mobile-segment-grid grid sm:grid-cols-2 xl:grid-cols-4 gap-2">
                ...
                <div className="mobile-segment-grid grid sm:grid-cols-2 xl:grid-cols-4 gap-2 mt-2 sm:mt-3 text-sm text-muted-foreground">
              ...
              {f.aiReason ? <p className="hidden sm:block text-xs text-primary mt-2">{f.aiReason}</p> : null}
            </div>
            <div className="mobile-action-row flex w-full flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-between lg:w-auto lg:flex-col lg:items-end">
              <div className="mobile-price-box text-right"><p className="text-xs text-muted-foreground">Total for {travelerCountNumber} traveler(s)</p><p className="font-bold text-lg sm:text-xl">{f.price ? formatCurrency(Number(f.price), f.currency || form.currency) : "No price"}</p>{f.price && travelerCountNumber > 1 ? <p className="text-xs text-muted-foreground">{formatCurrency(Math.round(Number(f.price) / travelerCountNumber), f.currency || form.currency)} per person</p> : null}</div>
          ...
          <Card><CardHeader><CardTitle className="flex items-center gap-2"><Hotel className="h-5 w-5" /> Live hotel options</CardTitle></CardHeader><CardContent className="space-y-3">
            {result.hotels?.length ? result.hotels.map((h: any, i: number) => { const active = selectedHotelId === h.id; const recommended = result.recommendedHotelId === h.id; return <div key={h.id || i} className={`mobile-result-card mobile-hotel-card rounded-2xl border p-4 sm:p-5 grid lg:grid-cols-[180px_1fr_auto] gap-3 sm:gap-4 transition-colors mobile-wrap ${active ? "border-primary bg-primary/10" : "hover:bg-muted/30"}`}>
              <div className="mobile-hotel-image rounded-xl bg-muted overflow-hidden min-h-28 sm:min-h-32 flex items-center justify-center">
                {h.image ? <img src={h.image} alt={h.name || "Hotel photo"} className="h-28 sm:h-32 w-full object-cover" /> : <Hotel className="h-10 w-10 text-muted-foreground" />}
              </div>
              <div>
                <div className="flex flex-wrap items-center gap-2"><p className="mobile-result-title font-semibold text-base sm:text-lg">{h.name}</p><Badge variant="secondary">{h.source || result.hotelSource || "Live Hotel API"}</Badge>{recommended && <Badge variant="success">AI recommended</Badge>}{active && <Badge variant="default">Selected</Badge>}</div>
                <div className="mobile-segment-grid grid sm:grid-cols-2 xl:grid-cols-4 gap-2 mt-2 sm:mt-3 text-sm text-muted-foreground">
                ...
                {h.aiReason ? <p className="hidden sm:block text-xs text-primary mt-2">{h.aiReason}</p> : null}
              </div>
              <div className="mobile-action-row flex w-full flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-between lg:w-auto lg:flex-col lg:items-end">
                <div className="mobile-price-box text-right"><p className="text-xs text-muted-foreground">Hotel total</p><p className="font-bold text-lg sm:text-xl">{h.price ? formatCurrency(Number(h.price), h.currency || form.currency) : "No price"}</p>{h.originalPrice && h.originalCurrency && h.originalCurrency !== (h.currency || form.currency) ? <p className="text-xs text-muted-foreground">Original: {h.originalPrice} {h.originalCurrency}</p> : null}</div>
